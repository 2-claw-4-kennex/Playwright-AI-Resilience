...
**Later, when the selector breaks:** The engine loads the stored fingerprint, scores up to 15 nearby DOM candidates using text, ARIA, structural, and spatial signals, picks the best match above the risk threshold, clicks it, and verifies the result.
...
```
  ...
  ↺  Scanning DOM neighborhood... 3 candidates found
...
```typescript
verify.urlTransition('/payment')           // Waits for URL match
...
tests/
  ...
  corpus/fixtures.ts    ← 10 breakage scenarios + 2 adversarial
